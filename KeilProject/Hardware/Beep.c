...
/*
一般尽量不选TIM1，因为是高级定时器，需要额外步骤
若不慎选择，更改时需要注意的点：
	1.TIM1用的是APB2总线，而其他通用计时器是APB1总线
	2.用TIM1需要TIM_CtrlPWMOutputs使能才能输出
*/
void Beep_Init(void)
{			
			/*若需要引脚重映射，使用下方函数*/
...
//			GPIO_PinRemapConfig(GPIO_PartialRemap1_TIM2, ENABLE);	//查表寻找重映射对应参数
	...
	//					分辨率=1/(ARR+1)
			...
			TIM_OC1Init(TIM1, &TIM_OCInitStructure);	//根据TIM的CH来配置
			/*同一TIM的多个CH可以继续加*/
			
			TIM_CtrlPWMOutputs(TIM1,ENABLE);//主输出使能，高级定时器使用，在使用高级定时器输出PWM时需调用并使能才能输出
...
{
			TIM_SetCompare1(TIM1, 50);
			TIM_PrescalerConfig(TIM1, 266, TIM_PSCReloadMode_Update);
}
